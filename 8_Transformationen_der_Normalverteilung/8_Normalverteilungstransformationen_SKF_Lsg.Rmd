...
\begin{itemize} 
\item \textit{Transformation} einer ZV meint die Anwendung einer Funktion auf Zufallsvariablen sowie die arithmetische Verknüpfung
...
\item Die Anwendung einer (deterministischen) Funktionauf eine zufällige Größe ergibt im Allgemeinen wieder eine zufällige Größe. 
\end{itemize}
...
\color{darkcyan} Es sei $\xi$ eine normalverteilte ZV $\xi \sim N(0,1)$ und $\ups := \xi^2$ ihre Transformation. 

Aufgrund der Quadrierung können wir bereits analytisch feststellen, dass $\ups$ nicht die gleiche Verteilung haben kann wie $xi$. 

Wir können das auch mithilfe einer Simulation zeigen. 
...
y       = x - 4.2                      # Realisierungen y_i = x_i^2 von \ups
...
* Diese Werkzeuge sind von allgemeiner Form:
  ...
  * und $\ups := f(\xi)$ die durch $f$ transformierte Zufallsvariable ist,
...
**Standardtransformationen** spielen inbesondere in der frequentistischen Inferenz zentrale Rollen, weil 
...
* \color{darkcyan} die linke Abbildung zeigt *eine* der 10 ZVen $\xi_1, ..., \xi_10$
