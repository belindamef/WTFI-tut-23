...
\center
\huge
\text{(6) Erwartungswert und Kovarianz}
...
\textcolor{darkcyan}{Per Definition ist $\mathcal{X} = \{0,1\}$ gegeben. Wenn wir sagen, dass das Ergebnis $0 \in \mathcal{X}$ Kopf und $1 \in \mathcal{X}$ Zahl repräsentiert, können wir mit einer Bernoullig-ZV mit $\mu = 0.5$ einen Münzwurf modellieren.}
...
# \textcolor{darkblue}{SKF 8}. *Kovarianz und Korrelation*

\large
\color{darkblue} 8. Definieren Sie die Kovarianz und Korrelation zweier Zufallsvariablen $\xi$ und $\ups$.
...
* Das Stichprobenmittel $\bar{\xi}_n$ ist eine Kennzahle einer Stichprobe $\xi_1,...,\xi_n$.
  ...
  * Verschiedenen Realisationen einer Stichprobe $\xi_1, ..., \xi_n$ können (und werden aller Wahrscheinlichkeit nach) verschiedene Werte für $\bar{\xi}_n$ ergeben. 
...
Da bei bei Unabhängigkeit $\mathbb{C}(\xi,\ups)=0$, ist die Varianz der Simmer zweier ZVen gegeben durch die Summe der Varianzen, formal
